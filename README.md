...
> Code: MIT. Spec: [Community Specification License 1.0](spec/LICENSE.md). "FALSIFY" name and chevron logo: unregistered marks of Falsify OÜ, ™ reserved. See [TRADEMARK.md](TRADEMARK.md) · [NOTICE](NOTICE) · [docs/COMMERCIAL.md](docs/COMMERCIAL.md).
...
- **[`spec/PRML-v0.1.md`](spec/PRML-v0.1.md)** — the spec (RFC-style, [Community Specification License 1.0](spec/LICENSE.md))
...
- **[IANA media type](https://www.iana.org/assignments/media-types/application/vnd.prml+yaml)** — `application/vnd.prml+yaml` is registered in the IANA media-types registry (vendor tree, 3 September 2026). Serve manifests with that `Content-Type`; the registration fixes encoding as `binary` so intermediaries must not re-encode. *(registry entry, not an endorsement)*
